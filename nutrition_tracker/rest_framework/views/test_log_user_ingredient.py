...
from nutrition_tracker.rest_framework.views import APILogUserIngredient
from nutrition_tracker.tests import constants as test_constants
from nutrition_tracker.tests import objects as test_objects


class TestViewsAPILogUserIngredient(APITestCase):
    @classmethod
    ...
    def test_unauthorized_post_fails(self):
        ...
        view = APILogUserIngredient.as_view()

        request = factory.post(reverse("api_log_user_ingredient", kwargs={"id": self.USER_INGREDIENT.external_id}))
    ...
    def test_log_authorized_request(self):
        ...
        request = factory.post(
            reverse("api_log_user_ingredient", kwargs={"id": self.USER_INGREDIENT.external_id}),
            {"external_id": self.USER_INGREDIENT.external_id, "meal_type": constants.MealType.LUNCH},
    ...
    def test_log_authorized_request_bad_meal_type(self):
        ...
        request = factory.post(
            reverse("api_log_user_ingredient", kwargs={"id": self.USER_INGREDIENT.external_id}),
            {"external_id": self.USER_INGREDIENT.external_id, "meal_type": constants.MealType.__empty__},
            HTTP_X_API_KEY=self.API_KEY,
        )
        force_authenticate(request, user=self.USER)
        response = view(request, id=self.USER_INGREDIENT.external_id)
    ...
    def test_log_authorized_mutation_request(self):
        ...
        request = factory.post(
            reverse("api_log_user_ingredient", kwargs={"id": self.USER_INGREDIENT.external_id}),
            {
                "external_id": self.USER_INGREDIENT.external_id,
    ...
    def test_log_authorized_request_id_not_found(self):
        ...
        request = factory.post(
            reverse("api_log_user_ingredient", kwargs={"id": test_constants.TEST_UUID_2}),
            {},
            HTTP_X_API_KEY=self.API_KEY,
        )
        force_authenticate(request, user=self.USER)
