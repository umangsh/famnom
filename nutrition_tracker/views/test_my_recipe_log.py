...
class TestViewsMyRecipeLog(TestCase):
    @classmethod
    ...
    def test_logged_out_redirects(self):
        response = self.client.get(reverse("my_recipe_log", kwargs={"id": self.USER_RECIPE.external_id}), follow=True)
        self.assertEqual(response.status_code, HTTPStatus.OK)
        self.assertRedirects(response, "/accounts/login/?next=/my_recipe_log/%s/" % self.USER_RECIPE.external_id)

    def test_logged_in_not_found_redirects(self):
        self.client.login(email="user@famnom.com", password="password")
        response = self.client.get(reverse("my_recipe_log", kwargs={"id": test_constants.TEST_UUID}), follow=True)
        self.assertEqual(response.status_code, HTTPStatus.OK)
    ...
    def test_logged_in_bad_id_redirects(self):
        self.client.login(email="user@famnom.com", password="password")
        response = self.client.get("/my_recipe_log/123")
        self.assertEqual(response.status_code, HTTPStatus.NOT_FOUND)
        self.assertTemplateUsed(response, "errors/404.html")

    def test_logged_in_no_meal_success(self):
        self.client.login(email="user@famnom.com", password="password")
        response = self.client.get(reverse("my_recipe_log", kwargs={"id": self.USER_RECIPE.external_id}))
        self.assertEqual(response.status_code, HTTPStatus.OK)
        self.assertTemplateUsed(response, "nutrition_tracker/my_recipe_log.html")
    ...
    def test_logged_in_meal_exists_post_success(self):
        ...
        response = self.client.post(
            reverse("my_recipe_log", kwargs={"id": self.USER_RECIPE.external_id}),
            {
        ...
        messages = [m.message for m in get_messages(response.wsgi_request)]
        self.assertIn(constants.MESSAGE_SUCCESS_RECIPE_LOG, messages)
        self.assertEqual(
            1,
